...
interface Props { theme?: 'dark' | 'light'; walletHint?: boolean; qrHint?: boolean; onWalletTap?: () => void; onQrTap?: () => void }
...
const infoRows = [
 { icon:'‍', label:'الطبيب المعالج', value:'د. سارة المطيري · طب عام' },
 { icon:'', label:'الأدوية النشطة', value:'ميتفورمين · أوميبرازول · فيتامين د' },
 { icon:'', label:'الحساسية', value:'بنسلين — لا حساسية أخرى' },
...
function WalletSheet({ onClose, days, hours, mins, secs: s, expiry }: any) {
 ...
 const handleAdd = () => {
 setLoading(true);
 window.location.href = '/api/wallet/pass?patientName=%D8%A3%D8%AD%D9%85%D8%AF+%D9%86%D8%A7%D8%B5%D8%B1+%D8%A7%D9%84%D8%B4%D9%85%D8%B1%D9%8A&patientId=PT-0842&clinicName=%D8%AA%D9%84%D9%82%D8%A7+%D8%A7%D9%84%D8%B9%D9%8A%D8%A7%D8%AF%D8%A7%D8%AA&bloodType=O%2B&insurance=%D8%A8%D9%88%D8%A8%D8%A7+%D9%A2%D9%A0%D9%A2%D9%A6&daysValid=7';
 setAdded(true);
 ...
 <div>
 <div className="flex items-center gap-1.5 mb-2"><TalqaShieldSmall size={12}/><p className="text-white/50 text-[10px] font-semibold">عيادة الشفاء</p></div>
 <p className="text-white text-[20px] font-bold leading-none">بطاقة مريض</p>
 <p className="text-white/25 text-[9px] mt-0.5">DIGITAL HEALTH CARD</p>
 </div>
 ...
 <div><p className="text-[8px] text-[#8E8E93] mb-0.5">PATIENT</p><p className="text-[16px] font-bold text-[#1C1C1E]">أحمد الشمري</p></div>
 <div className="text-left"><p className="text-[8px] text-[#8E8E93] mb-0.5">BLOOD</p><p className="text-[16px] font-bold text-[#1C1C1E]">O+</p></div>
 </div>
 ...
 <p className="text-[7px] text-[#8E8E93] mb-0.5">{l}</p>
...
export function ScreenCard({ theme = 'dark', walletHint = false, qrHint = false, onWalletTap, onQrTap }: Props) {
 ...
 <p className="text-white/20 text-[7px] mt-0.5">DIGITAL HEALTH</p>
 </div>
 </div>
 ...
 <div className="mb-5">
 <p className="text-white/20 text-[8px] mb-1">PATIENT NAME</p>
 ...
 <p className="text-white/25 text-[7px] mb-1">{l}</p>
 ...
 {qrHint && [1,2].map(n => (
  <motion.div key={n} className="absolute inset-0 rounded-[14px] pointer-events-none z-10"
   style={{ border:`${n===1?2:1.5}px solid rgba(0,180,216,${n===1?0.85:0.4})` }}
   animate={{ scale:[1,1.12,1], opacity:[0.9,0,0.9] }}
   transition={{ duration:1.3, repeat:Infinity, delay:n*0.3, ease:'easeOut' }}/>
 ))}
 {qrHint && (
  <motion.div className="absolute -top-9 left-1/2 -translate-x-1/2 z-20 whitespace-nowrap px-3 py-1.5 rounded-full text-[10px] font-black text-white pointer-events-none flex items-center gap-1.5"
   style={{ background:'linear-gradient(135deg,#0077A8,#00B4D8)', boxShadow:'0 4px 14px rgba(0,180,216,0.5)' }}
   animate={{ y:[0,-4,0] }} transition={{ duration:1.1, repeat:Infinity }}>
   <span>اضغط لعرض QR فوري</span>
  </motion.div>
 )}
 <motion.button whileTap={{ scale:0.96 }} onClick={() => setShowQR(true)}
  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-[14px] text-[11px] font-bold relative z-0"
  style={{ background: qrHint ? 'rgba(0,180,216,0.18)' : qrBg, border:`1px solid ${qrHint ? 'rgba(0,180,216,0.55)' : qrBdr}`, color: qrColor }}>
 ...
 {/* بطاقة المريض */}
 <div className="relative">
 {walletHint && [1,2].map(n => (
  <motion.div key={n} className="absolute inset-0 rounded-[16px] pointer-events-none z-10"
   style={{ border:`${n===1?2:1.5}px solid rgba(255,255,255,${n===1?0.65:0.28})` }}
   animate={{ scale:[1,1.08,1], opacity:[0.9,0,0.9] }}
   transition={{ duration:1.3, repeat:Infinity, delay:n*0.3, ease:'easeOut' }}/>
 ))}
 {walletHint && (
  <motion.div className="absolute -top-9 left-1/2 -translate-x-1/2 z-20 whitespace-nowrap px-3 py-1.5 rounded-full text-[10px] font-black text-black pointer-events-none flex items-center gap-1.5"
   style={{ background:'#fff', boxShadow:'0 4px 14px rgba(0,0,0,0.3)' }}
   animate={{ y:[0,-4,0] }} transition={{ duration:1.1, repeat:Infinity }}>
   <svg width="10" height="12" viewBox="0 0 17 20" fill="black"><path d="M14.1 10.64c-.02-2.04 1.67-3.02 1.74-3.06-0.95-1.39-2.43-1.58-2.95-1.60-1.26-.13-2.46.74-3.10.74-.64 0-1.63-.72-2.68-.70C5.55 6.04 4.05 6.97 3.22 8.36 1.54 11.17 2.80 15.35 4.42 17.65c.80 1.15 1.77 2.45 3.04 2.40 1.22-.05 1.68-.78 3.16-.78 1.47 0 1.89.78 3.18.76 1.31-.02 2.15-1.18 2.94-2.34.93-1.34 1.32-2.64 1.34-2.71-.03-.01-2.57-.99-2.98-3.34zM11.96 3.83c.67-.81 1.12-1.93 1.00-3.06-.96.04-2.13.64-2.82 1.45-.62.71-1.16 1.86-1.01 2.96 1.07.08 2.16-.54 2.83-1.35z"/></svg>
   أضف لـ Apple Wallet
  </motion.div>
 )}
 <motion.button whileTap={{ scale:0.97 }} onClick={() => { setShowWallet(true); onWalletTap?.(); }}
 ...
 <p className="text-[11px] font-bold uppercase mb-2.5" style={{ color: txtSub }}>ملفي الطبي</p>
