  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch('/api/wallet/pass', {
        ...
        body: JSON.stringify({ patientName:'أحمد ناصر الشمري', patientId:'PT-0842', clinicName:'عيادة الشفاء', bloodType:'O+', insurance:'بوبا ٢٠٢٦', daysValid:7 }),
      });
      if (res.ok) { const blob = await res.blob(); window.location.href = URL.createObjectURL(blob); }
    } catch {}
    setLoading(false); setAdded(true); setTimeout(onClose, 2000);
  };
      ...
      <motion.div initial={{ y:'100%' }} animate={{ y:0 }} exit={{ y:'100%' }}
        ...
        <div className="mx-4 mb-4 rounded-[20px] overflow-hidden"
          ...
          <div className="px-5 pt-5 pb-4 relative"
            style={{ background:'linear-gradient(135deg,#060E1C,#0B3A5A,#0B4A8A)' }}>
            <div className="absolute inset-0 pointer-events-none"
              style={{ background:'radial-gradient(ellipse at 80% 20%,rgba(0,180,216,0.22) 0%,transparent 60%)' }}/>
            <div className="relative z-10 flex items-start justify-between">
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  <Shield size={10} style={{ color:'#00B4D8' }}/>
                  <p className="text-white/50 text-[10px] font-semibold tracking-wider">عيادة الشفاء</p>
                </div>
              ...
              <div className="flex flex-col items-end gap-1">
                <div className="px-2.5 py-1 rounded-full"
                  style={{ background: days<=2 ? 'rgba(239,68,68,0.2)' : 'rgba(0,180,216,0.15)', border:`1px solid ${days<=2 ? 'rgba(239,68,68,0.35)' : 'rgba(0,180,216,0.3)'}` }}>
                  <p className="text-[9px] font-black" style={{ color: days<=2 ? '#FCA5A5' : '#00B4D8' }}>
                    {days} أيام متبقية
                  </p>
                </div>
                <p className="text-white/25 text-[8px] font-mono">
                  {String(hours).padStart(2,'0')}:{String(mins).padStart(2,'0')}:{String(s).padStart(2,'0')}
                </p>
              </div>
            </div>
          </div>
          <div className="bg-white px-5 py-4">
            ...
            <div className="flex items-center gap-3">
              <QRCodeSvg/>
              <div>
                <p className="text-[8px] text-[#8E8E93] mb-1">رمز الاستقبال الفوري</p>
                <p className="text-[10px] font-mono font-bold text-[#1C1C1E]">PT-2024-0842</p>
                <p className="text-[8px] text-[#8E8E93] mt-1 leading-relaxed">أرِ هذه البطاقة لموظف الاستقبال</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <motion.button whileTap={{ scale:0.97 }} onClick={loading ? undefined : handleAdd}
                ...
                {loading
                  ? <><motion.div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white"
                      ...
                      <span className="text-white font-semibold text-[14px]">جارٍ التحميل…</span></>
            ...
            <motion.div key="done" initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} className="px-4 pb-8">
              <div className="w-full flex items-center justify-center gap-2 py-4 rounded-[18px]"
                ...
                <motion.span initial={{ scale:0 }} animate={{ scale:1 }} transition={{ type:'spring', damping:12 }}
                  className="text-[#34C759] text-[18px]">✓</motion.span>
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <motion.div initial={{ scale:0.85, opacity:0 }} animate={{ scale:1, opacity:1 }} exit={{ scale:0.9, opacity:0 }}
        ...
        style={{ background:'#141E2E', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 24px 60px rgba(0,0,0,0.5)' }}
        onClick={e => e.stopPropagation()}>
        <div className="px-6 pt-5 pb-4 relative overflow-hidden"
          style={{ background:'linear-gradient(135deg,#060E1C,#0B3A5A)' }}>
          <div className="absolute inset-0 pointer-events-none"
            style={{ background:'radial-gradient(ellipse at 50% 0%,rgba(0,180,216,0.2) 0%,transparent 60%)' }}/>
        ...
        <div className="px-6 pt-5 pb-6 flex flex-col items-center">
          <div className="p-5 rounded-[20px] mb-3"
            style={{ background:'linear-gradient(145deg,#060E1C,#0B3A5A)', border:'1px solid rgba(0,180,216,0.15)' }}>
            <QRCodeSvg light/>
          </div>
          <p className="text-white/30 text-[11px] font-mono mb-4">PT-2024-0842</p>
          <div className="relative w-28 h-0.5 rounded-full overflow-hidden mb-5"
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto scrollbar-none"
      ...
      <div className="shrink-0 px-5 pt-3 pb-5 relative overflow-hidden"
        ...
        <div className="absolute inset-0 pointer-events-none"
          style={{ background:'radial-gradient(ellipse at 70% 30%,rgba(0,180,216,0.15) 0%,transparent 60%)' }}/>
        ...
        <div className="relative z-10">
          <p className="text-white/30 text-[10px] mb-1">رقمية · آمنة · دائمة</p>
      ...
      <div className="px-4 mt-3 mb-4">
        <motion.div initial={{ opacity:0, y:14, scale:0.97 }} animate={{ opacity:1, y:0, scale:1 }}
          ...
          style={{ background:'linear-gradient(145deg,#060E1C,#0B3558,#0C4070)', border:'1px solid rgba(0,180,216,0.2)', boxShadow:'0 16px 48px rgba(0,0,0,0.4)' }}>

          <div className="absolute inset-0 pointer-events-none"
            style={{ background:'radial-gradient(ellipse at 80% 10%,rgba(0,180,216,0.18) 0%,transparent 60%)' }}/>
          {/* Shimmer */}
          ...
          <div className="relative z-10 p-5">
            <div className="flex items-start justify-between mb-5">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl flex items-center justify-center"
                  style={{ background:'rgba(0,180,216,0.15)', border:'1px solid rgba(0,180,216,0.2)' }}>
            ...
            <div className="grid grid-cols-3 gap-2 mb-5">
              {[['PATIENT ID','#PT-0842','#00B4D8'],['BLOOD TYPE','O+','#fff'],['INSURANCE','بوبا ٢٠٢٦','rgba(255,255,255,0.65)']].map(([l,v,c]) => (
                <div key={l} className="rounded-[14px] px-3 py-2.5"
            ...
            <motion.button whileTap={{ scale:0.96 }} onClick={() => setShowQR(true)}
              ...
              style={{ background:'rgba(0,180,216,0.12)', border:'1px solid rgba(0,180,216,0.22)', color:'#00B4D8' }}>
      ...
      <div className="px-4 mb-4">
        <motion.button whileTap={{ scale:0.97 }} onClick={() => setShowWallet(true)}
          ...
          style={{ background:'#000', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 4px 20px rgba(0,0,0,0.3)' }}>
          <svg width="14" height="17" viewBox="0 0 17 20" fill="white"><path d="M14.1 10.64c-.02-2.04 1.67-3.02 1.74-3.06-0.95-1.39-2.43-1.58-2.95-1.60-1.26-.13-2.46.74-3.10.74-.64 0-1.63-.72-2.68-.70C5.55 6.04 4.05 6.97 3.22 8.36 1.54 11.17 2.80 15.35 4.42 17.65c.80 1.15 1.77 2.45 3.04 2.40 1.22-.05 1.68-.78 3.16-.78 1.47 0 1.89.78 3.18.76 1.31-.02 2.15-1.18 2.94-2.34.93-1.34 1.32-2.64 1.34-2.71-.03-.01-2.57-.99-2.98-3.34zM11.96 3.83c.67-.81 1.12-1.93 1.00-3.06-.96.04-2.13.64-2.82 1.45-.62.71-1.16 1.86-1.01 2.96 1.07.08 2.16-.54 2.83-1.35z"/></svg>
      ...
      {/* Info rows */}
      <div className="px-4 mb-4">
        <p className="text-white/60 text-[11px] font-bold uppercase tracking-widest mb-2.5">ملفي الطبي</p>
          ...
          {infoRows.map((r, i) => (
            <div key={i} className="flex items-center gap-3 px-4 py-3.5"
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white/30 text-[9px] mb-0.5">{r.label}</p>
                <p className="text-white text-[11px] font-semibold leading-snug">{r.value}</p>
              </div>
              <ChevronLeft size={13} className="text-white/15 shrink-0"/>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 mb-6">
        <div className="rounded-[18px] px-4 py-3.5 flex items-center gap-3"
          ...
          <div>
            <p className="text-[#FF6B6B] text-[12px] font-bold">بيانات الطوارئ</p>
            <p className="text-white/30 text-[10px]">تظهر على شاشة القفل عند الطوارئ</p>
          </div>
        </div>
      </div>
