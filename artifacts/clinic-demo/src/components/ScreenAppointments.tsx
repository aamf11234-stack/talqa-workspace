...
const statusCfg = {
  ...
  done:      { label:'مكتمل',         bg:'rgba(255,255,255,0.06)', color:'rgba(255,255,255,0.3)', border:'rgba(255,255,255,0.08)' },
};
    ...
    <div className="flex flex-col h-full" style={{ background:'#0E1621', fontFamily:'Tajawal,sans-serif' }}>
      ...
      <div className="shrink-0 px-5 pt-3 pb-5 relative overflow-hidden"
        ...
        <div className="relative z-10">
          <p className="text-white/30 text-[10px] mb-1">إدارة جداولك</p>
          <p className="text-white text-[20px] font-bold mb-4">مواعيدي</p>
          {/* Tab switcher */}
                  ...
                  <div className="px-4 py-4 flex items-start gap-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-1.5">
                        <div>
                          <p className="text-white text-[13px] font-bold leading-none mb-1">{a.dr}</p>
                          <p className="text-white/35 text-[10px]">{a.spec}</p>
                        </div>
                    ...
                    <ChevronLeft size={13} className="text-white/15 shrink-0 mt-1"/>
                  </div>
                </motion.div>
              );
            })}
        ...
        {booking && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            <motion.div initial={{ y:500 }} animate={{ y:0 }} exit={{ y:500 }}
              ...
              style={{ background:'#141E2E', border:'1px solid rgba(255,255,255,0.08)', borderBottom:'none', boxShadow:'0 -8px 40px rgba(0,0,0,0.4)' }}>
              ...
              {done ? (
                <div className="flex flex-col items-center py-10 px-6">
                  ...
                  <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2 }} className="text-center">
                    <p className="text-white text-[18px] font-bold mb-1.5">تم تسجيل موعدك!</p>
                    <p className="text-white/40 text-[12px] leading-relaxed mb-6">{selSpec} · {selTime}<br/>ستصلك رسالة تأكيد على الواتساب</p>
                      ...
                      <div className="flex gap-1.5 mb-1.5">
                        {[0,1].map(s => (
                          <motion.div key={s} className="h-1 rounded-full"
                            animate={{ width: step>=s ? 28 : 18, background: step>=s ? '#00B4D8' : 'rgba(255,255,255,0.12)' }}
                            transition={{ duration:0.3 }}/>
                        ))}
                      </div>
                      <p className="text-white text-[14px] font-bold">{step===0 ? 'اختر التخصص' : 'اختر الوقت'}</p>
                    </div>
                          ...
                          {specialties.map(s => (
                            <motion.button key={s} whileTap={{ scale:0.94 }} onClick={() => setSelSpec(s)}
                              ...
                              style={selSpec===s
                                ...
                                : { background:'rgba(255,255,255,0.06)', color:'rgba(255,255,255,0.5)', border:'1px solid rgba(255,255,255,0.07)' }}>
                              {s}
                            </motion.button>
                          ))}
                        </div>
