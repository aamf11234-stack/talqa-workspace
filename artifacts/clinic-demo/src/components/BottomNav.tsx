...
interface Props {
 ...
 notifCount?: number;
 theme?: 'dark' | 'light';
 hintTab?: ClinicTab | null;
}

export function BottomNav({ activeTab, onChangeTab, notifCount = 2, theme = 'dark', hintTab }: Props) {
  ...
  {tabs.map((tab) => {
   ...
   return (
    ...
    >
     ...
     {/* Tour hint pulse rings */}
     {isHinted && [1,2].map(n => (
      <motion.div key={n}
       className="absolute inset-0 rounded-[18px] pointer-events-none"
       style={{ border: `${n===1?2:1.5}px solid rgba(0,180,216,${n===1?0.7:0.35})` }}
       animate={{ scale: [1, 1.18, 1], opacity: [0.9, 0, 0.9] }}
       transition={{ duration: 1.4, repeat: Infinity, delay: n * 0.3, ease: 'easeOut' }}
      />
     ))}

     {/* Hint glow background */}
     {isHinted && (
      <motion.div
       className="absolute inset-0 rounded-[18px] pointer-events-none"
     ...
     <div className="relative z-10 flex flex-col items-center gap-0.5">
      <Icon
       size={isActive ? 17 : 16}
       style={{ color: isActive ? activeColor : isHinted ? '#00B4D8' : iconOff, transition: 'all 0.2s' }}
      />
      <span
       className="text-[8px] font-bold leading-none"
       style={{ color: isActive ? activeColor : isHinted ? '#00B4D8' : labelOff, transition: 'color 0.2s' }}
      >
       {tab.label}
      </span>
     </div>
