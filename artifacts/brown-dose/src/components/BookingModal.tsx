...
import {
  X, ChevronRight, ChevronLeft, Check, Coffee, Zap, Crown,
  Building2, MapPin, User, Phone, Sparkles, Clock, Headphones, Rocket
} from 'lucide-react';
...
/* ─── Package card ─── */
function PkgCard({
  id, icon: Icon, title, price, features, tag, selected, onSelect,
}: {
  id: 'starter' | 'pro';
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background: selected ? 'rgba(196,120,58,0.1)' : 'rgba(255,255,255,0.03)',
    ...
    >
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', marginRight: 4 }}>ريال</span>
      </div>
...
export function BookingModal({ onClose }: { onClose: () => void }) {
  ...
  const [data, setData] = useState<BookingData>({
    pkg: '', bizType: '', bizName: '', city: '', name: '', phone: '',
  });
                ...
                {step === 0 && (
                  <motion.div key="s0" custom={dir} variants={slide} initial="initial" animate="animate" exit="exit" transition={slide.transition}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      <PkgCard
                        id="starter" icon={Coffee} title="باقة الكافيه" price="٢٬٠٠٠"
                        features={['تطبيق ويب بهوية نشاطك', 'نظام نقاط ولاء تلقائي', 'قائمة QR تفاعلية', 'دعم واتساب ٦ أشهر']}
                        selected={data.pkg === 'starter'} onSelect={() => set('pkg')('starter')}
                      />
                      <PkgCard
                        id="pro" icon={Crown} title="باقة المطعم الكامل" price="٣٬٥٠٠"
                        features={['كل ميزات الكافيه', 'Apple & Google Wallet', 'نظام توصيل واستلام', 'لوحة تحكم + إحصائيات', 'تطبيق iOS + Android']}
                        tag="الأكثر طلباً ✦"
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      <div style={{ background: 'rgba(196,120,58,0.08)', border: `1px solid ${C.border}`, borderRadius: 14, padding: '14px 16px', marginBottom: 4 }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#fff' }}>
                          {data.pkg === 'pro' ? '👑 باقة المطعم الكامل — ٣,٥٠٠ ريال' : '☕ باقة الكافيه — ٢,٠٠٠ ريال'}
                        </div>
                        {data.bizName && <div style={{ fontSize: 12, color: 'rgba(232,160,96,0.7)', marginTop: 3 }}>{data.bizName} · {data.city || 'المملكة'}</div>}
                      </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              {step > 0 && (
                <button onClick={back} style={{
                  ...
                  display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'Noto Kufi Arabic, Cairo, sans-serif',
                }}>
    ...
    <>
      <button
        ...
        style={{
          ...(variant === 'primary' ? {
            background: C.grad, color: '#fff',
            boxShadow: '0 8px 28px rgba(196,120,58,0.35)',
            border: 'none',
          } : {
