...
export function HomeScreen() {
  const { points, setActiveTab, userName, businessName } = useAppContext();
  const [animatedPoints, setAnimatedPoints] = useState(0);

  useEffect(() => {
    const t = setTimeout(() => setAnimatedPoints(points), 300);
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 flex items-center justify-center">
        <div className="text-xs font-bold tracking-wider opacity-40 uppercase">{businessName}</div>
      </div>

      {/* Loyalty Ring */}
      <div className="relative flex justify-center items-center py-4 mb-8">
        <svg className="w-40 h-40 transform -rotate-90">
          <circle cx="80" cy="80" r="70" fill="none" stroke="hsl(var(--card))" strokeWidth="6" />
          <motion.circle
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        <motion.button whileTap={{ scale: 0.95 }} onClick={() => setActiveTab('menu')}
      ...
      <div className="mb-6">
        ...
        <motion.div whileTap={{ scale: 0.98 }} className="relative h-48 rounded-3xl overflow-hidden shadow-xl">
          <img src={specialtyImage} alt="Specialty Coffee" className="absolute inset-0 w-full h-full object-cover" />
          ...
          <div className="absolute bottom-4 right-4 left-4 flex justify-between items-end">
            <div>
              ...
              <h4 className="text-white font-bold text-lg">سيزونال سبيشل ✦</h4>
            </div>
