...
function MobileCard({ card, i }: { card: typeof CARDS[0]; i: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <div style={{ position: 'absolute', bottom: -8, right: 20, fontSize: 80, fontWeight: 900, color: 'rgba(255,255,255,0.025)', letterSpacing: '-0.06em', lineHeight: 1, userSelect: 'none' }}>{card.num}</div>

      {card.tag && (
        <div style={{ position: 'absolute', top: 20, left: 20, fontSize: 9, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: 6, background: `${card.accent}18`, color: card.accent, border: `1px solid ${card.accent}35` }}>
          {card.tag}
        </div>
      )}

      <div style={{ width: 48, height: 48, borderRadius: 13, marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${card.accent}12`, color: card.accent, border: `1px solid ${card.accent}22` }}>
        <card.Icon size={20} strokeWidth={1.5} />
      </div>

      <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.2)', marginBottom: 6 }}>{card.en}</div>
      <h3 style={{ fontSize: 18, fontWeight: 900, color: '#fff', letterSpacing: '-0.025em', marginBottom: 12, lineHeight: 1.2 }}>{card.ar}</h3>
      <p style={{ fontSize: 13, lineHeight: 1.85, color: 'rgba(255,255,255,0.4)', marginBottom: 18 }}>{card.desc}</p>
    ...
    <div ref={containerRef} style={{ height: `${CARDS.length * 80}vh`, position: 'relative' }}>
      <div style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden', background: 'var(--bg)' }}>
        ...
        <div style={{ position: 'absolute', top: 0, inset: '0 0 auto', zIndex: 2, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', padding: '36px 52px 0', pointerEvents: 'none' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
            <motion.div style={{ width: 5, height: 5, borderRadius: '50%', background: '#4F8EFF' }} animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 1.8, repeat: Infinity }} />
          ...
          <motion.div style={{ display: 'flex', gap: 20, x, willChange: 'transform' }}>
            {CARDS.map((card, i) => (
              <motion.div key={i}
                ...
                style={{ flexShrink: 0, width: 340, height: '65vh', maxHeight: 560, background: 'var(--surface)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: '36px 32px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', transition: 'border-color 0.4s, box-shadow 0.4s' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = `${card.accent}40`; (e.currentTarget as HTMLElement).style.boxShadow = `0 0 0 1px ${card.accent}20, 0 32px 80px rgba(0,0,0,0.5)`; }}
...
export default function HorizontalServices() {
  ...
  if (isMobile) {
    return (
      <section id="services" style={{ padding: '72px 0', background: 'var(--bg)', position: 'relative' }}>
        <div style={{ position: 'absolute', top: 0, inset: '0 0 auto', height: 1, background: 'rgba(255,255,255,0.07)' }} />
        <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 18px' }}>
          <div style={{ marginBottom: 36 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#4F8EFF', marginBottom: 12 }}>ما نبنيه لك</div>
            <h2 style={{ fontWeight: 900, fontSize: '1.9rem', letterSpacing: '-0.03em', color: '#fff', lineHeight: 1.1 }}>
              ستة حلول.<br /><span className="text-blue">منظومة واحدة.</span>
            </h2>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {CARDS.map((card, i) => <MobileCard key={i} card={card} i={i} />)}
          </div>
        </div>
      </section>
    );
